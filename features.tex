...
The process of encoding each data point as percepts/features is called \textbf{extraction}.
...
\subsection{Instantiation and recognition}

Feature extraction code has to perform two tasks:
\begin{itemize}
  \item \textbf{Instantiation:} Decide which percepts/features will be included in the feature function used by the model. 
  ...
  \item \textbf{Recognition:} Given the model's feature function, enumerate the active percepts/features and their values.
  This is performed for each instance during training\footnote{To avoid repeating extraction on the training data 
during each iteration of learning, consider caching each instance's percepts/features (see \nss{TODO}).} and at test time.
\end{itemize}
...
%\paragraph{Cutoffs.}
Sometimes a \textbf{cutoff} is used to remove infrequent percepts/features as measured by the training data.
Active percepts/features are counted during instantiation, and then any occurring fewer than $k$ times 
...
\nss{implementation issues: data types/structures; store features in memory/disk? separate percepts from features when using the cross-product parametrization; 
feature hashing; etc.}

\paragraph{Background label.} choose a background label (\Sref{sec:bg})

\paragraph{String indexing.} store categorical percepts as integers rather than strings

\paragraph{Unpacking.} instead of storing complex percepts as a string (or indexed string), 
use a tuple of atomic percepts

\paragraph{Implicit feature indexing.} with the cross-product parametrization, features can be 
computed on the fly from percept and label indices
