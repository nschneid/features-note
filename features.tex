...
\nss{we do not address conceptual design/selection of features, or techniques for helping automate 
the choice of features using the model's training data and/or auxiliary data 
(dimensionality reduction, feature selection, structured sparsity, multitask learning).}
...
\section{Beyond simple classification: leveraging structure}
...
any nonzero domain-specific features will help ``customize'' the prediction for the domain.\footnote{\Citet{daume-07} 
...
This simple technique often works well, the only caveat being that greatly expanding the number of parameters 
means that regularization (\cref{sec:reg}) becomes more important to avoid overfitting.
...
\subsection{Structure in the label (type) space}

\nss{hierarchical classification; attributes in COLING definiteness paper. features, cost function, regularization}
