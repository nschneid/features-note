...
a negative score means it is a worse fit).\nss{how does this translate to logistic regression?} Designating the background label $\mathrm{Y}^{\textsc{bg}}$, the classifier can then be rewritten as
...
\begin{equation}
\nabla_j = -f_j(x,y) + f_j(x,\left(\argmax_{y'}{\boldsymbol{\theta}^\top\mathbf{f}(x,y')+\textit{cost}(y,y')}\right))
\end{equation}
...
\nss{is there weirdness regularizing with max margin loss? just scaling?}
...
\hline
generative, probabilistic & multinomial na\"{i}ve Bayes (\Sref{sec:naive}) & {\raggedright multinomial discrete hidden Markov model (HMM)} \\
\hline
...
\begin{enumerate}
  ...
  \item Implement the impact measure given in \Sref{sec:interp} 
