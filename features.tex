...
We assume that $\mathbf{f}$ returns $d$ components---$\mathbf{f}: \mathcal{X}\times\mathcal{Y}\rightarrow\mathbb{R}^d$.

The scoring function can then depend on the sum of the components of the vector returned by $\mathbf{f}$ weighted by corresponding {\bf parameters}.\footnote{The parameters in a model are sometimes called its {\bf coefficients} or {\bf weights}. 
