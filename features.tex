...
If the goal is to predict the single best label, the \textbf{max-margin loss} is a popular alternative:\footnote{The \textbf{cost function} assigns a nonnegative penalty to erroneous predictions.
...
$\sum_{(x,y)\in\mathcal{D}}{\lsem \left(\max_{y'}{\boldsymbol{\theta}^\top\mathbf{f}(x,y')}\right)\neq y \rsem}$.\nss{right?}
...
\end{equation}
Its subgradient with respect to the $j$th parameter is
\begin{equation}
\nabla_j = 
\end{equation}
